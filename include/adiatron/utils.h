...
inline void printHelp(int argc, char* argv[]) {
  ...
  std::cout
    ...
    << "\tadd \t\t Add file or directory to encrypted archive.\n"
    << "\tkeygen \t\t Generate a new public/secret keypair. Note that without --passphrase flag secret key is unencrypted.\n"
    ...
    << "\t" << name << " add archive.enc file.txt\n"
    << "\t" << name << " add archive.enc direcorty/\n"
    << "\t" << name << " list archive.enc\n"
    << "\t" << name << " keygen\n"
    << "\t" << name << " keygen --passphrase\n"
    << "\n"
    << "Options:\n"
